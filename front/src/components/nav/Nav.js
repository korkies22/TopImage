    ...
    <nav className={`nav ${isMobile?'nav--active':''}`}>
      ...
      <div className={`${isMobile?'nav__actions--active':'nav__actions'}`}>
        ...
        <div className="nav__user--responsive">
          <NavLink exact={true} className="nav__link" activeClassName='nav__link--active' to="/tutorial">Tutorial</NavLink>
          <button onClick={logout} className="nav__mockLink" >Log Out</button>
        </div>
      </div>
